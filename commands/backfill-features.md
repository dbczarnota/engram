---
description: Seed feature notes for an existing project by distilling its wiki + specs + CRG code graph into features/_inbox drafts for review. Usage: /backfill-features <slug | current>
---
...
1. **Resolve** the slug + repo path from `_meta/project-map.json` (arg or cwd). If
   ...
   page must exist before backfilling features).
...
3. **Gather sources (read-only):**
   ...
   - CRG's architecture overview (MCP `get_architecture_overview` / `get_hub_nodes` / `list_communities`)
     — hub nodes + communities give code-level boundaries and the file groupings for `files:`. If CRG is
     absent, inspect the repo structure directly.
...
5. **For each chosen candidate**, write a draft `projects/<slug>/features/_inbox/<YYYY-MM-DD>-<name>.md`
   ...
   - frontmatter graph keys: `standards`/`lessons` (wikilinks where evident), `spec` (the driving spec
     ...
     JSON array** of the feature's repo-relative paths (from CRG hub nodes / the code area).
